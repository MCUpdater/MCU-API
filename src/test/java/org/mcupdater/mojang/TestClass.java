...
public class TestClass {
	...
	public static void main(String[] args) throws MalformedURLException {
		...
		*/
		...
		HashSet<Downloadable> libSet = new HashSet<>();
		...
		synchronized(q2.getSuccesses()) {
			for (Downloadable d : q2.getSuccesses()) {
				classpath.append(MCUpdater.cpDelimiter()).append(new File(new File(base, "lib"), d.getFilename()).getAbsolutePath());
			}
		}
		synchronized(q3.getSuccesses()) {
			for (Downloadable d : q3.getSuccesses()) {
				classpath.append(MCUpdater.cpDelimiter()).append(new File(base, d.getFilename()).getAbsolutePath());
			}
		}
		...
		System.out.println(procArgs.toArray(new String[procArgs.size()]));
